...
async def clean_azure_test_graph_data():
    ...
    Also cleans up Postgres Alert rows generated by AlertGenerator during the
    scan - added after the detection->alert pipeline went live (Week 8 Item 1),
    since a real Alert row now gets created every time this test runs and
    nothing else was cleaning it up."""
    from graph.neo4j_client import Neo4jClient
    from conftest import wipe_nodes_matching
    from db.database import SessionLocal
    from db.models import Alert

    def _wipe_test_alerts():
        db = SessionLocal()
        try:
            db.query(Alert).filter(
                Alert.source_node_id.like(f"{AZURE_TEST_MARKER}%")
